(async function () {
    try {
        ...
        function updateTopBoxByZoom() {
            if (!topBox) return;

            // zoomFactor: 1 = 원래 상태, 값이 클수록 더 확대된 상태
            const zoomFactor = originalViewBox.w / viewBox.w;

            // 구간 설정
            const fadeStart = 1.1;   // 불투명 → 투명 시작
            const fadeEnd = 4.0;   // 완전히 투명해지는 시점
            const slideStart = fadeEnd; // 투명해진 직후 이동 시작
            const slideEnd = 4;   // 완전히 올라가서 사라짐
            ...
            // ① Fade-out(투명해지는) 구간
            // --------------------------
            if (zoomFactor <= fadeStart) {
                opacity = 1;  // 완전 보임
            }
            else if (zoomFactor >= fadeEnd) {
                opacity = 0;  // 완전 투명
            }
            else {
                opacity = 1 - (zoomFactor - fadeStart) / (fadeEnd - fadeStart);
            }

            // --------------------------
            // ② Slide-out(위로 올라가는) 구간
            //    투명해진 이후에만 이동 시작
            // --------------------------
            if (zoomFactor <= slideStart) {
                translateY = 0;       // 이동 없음
            }
            else if (zoomFactor >= slideEnd) {
                translateY = -100;    // 완전 위로 이동
            }
            else {
                const t = (zoomFactor - slideStart) / (slideEnd - slideStart);
                translateY = -100 * t;  // 0 → -100% 사이 부드럽게 이동
            }

            // 적용
            topBox.style.opacity = opacity;
            topBox.style.transform = `translateY(${translateY}%)`;

            // 완전히 사라졌으니 이벤트 막지 않기
            topBox.style.pointerEvents = opacity === 0 ? 'none' : 'auto';
        }


        // ★ 초기 한 번 호출 (처음엔 그대로 보이게)
        updateTopBoxByZoom();
